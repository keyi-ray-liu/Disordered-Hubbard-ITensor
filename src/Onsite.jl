...
function Onsite(sys::Ring, j::Int; left_offset=0)

    onsite = Onsite(sys.array, j; left_offset=left_offset)

    Lx = sys.array.Lx
    Ly = sys.array.Ly
    adj_j = j - left_offset

    row = div(adj_j - 1, Lx) + 1
    col = adj_j % Lx

    if 1 < row < Ly  && 1 < col < Lx
        onsite += sys.central_potential
    end 
